# -*- coding: utf-8 -*-
"""
Created on Thu Jan 30 14:02:53 2020
    ...
    def GetFaceSearchCollectionResults(self):
        ...
        while finished == False:
            ...
            for personMatch in response['Persons']:
                ...
                ##json.dump(str(personMatch['Person']['Index']), fp)
                #json.dump(str(personMatch['Timestamp']), fp)
                tim = str(personMatch['Timestamp']/1000)
                print(tim)
                ...
                if ('FaceMatches' in personMatch):
                    for faceMatch in personMatch['FaceMatches']:
                        ...
                        json.dump(personMatch, fp)
                        getFrame(personMatch['Timestamp'])
                        ##json.dump(str(faceMatch['Similarity']), fp)
                print()
            if 'NextToken' in response:
                paginationToken = response['NextToken']
            else:
...
vidcap = cv2.VideoCapture('id2.mp4')
def getFrame(sec):
    print('inside fc')
    print(sec)
    ...
    if hasFrames:
        print('hasframes')
        cv2.imwrite("image"+str(sec)+".jpg", image)     # save frame as JPG file
    return hasFrames
...
def main():
    roleArn = 'arn:aws:iam::125733266075:role/vidrole'   
